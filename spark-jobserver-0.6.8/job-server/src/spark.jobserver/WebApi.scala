...
//                notFound(ctx, "No running job with ID " + jobId.toString)
              //modify by liug 20171220
              case JobInfo(_, contextName, _, classPath, _, _, Some(ex)) =>
                logger.info("### user delete ERROR job : " + jobId)
                jobInfo ! RemoveJobInfo(jobId)
                logger.info("### DONE ! user deleted ERROR job : " + jobId)
                ctx.complete(Map(StatusKey -> "DELETED"))
              case JobInfo(_, contextName, _, classPath, _, Some(e), None) =>
                logger.info("### user delete job : " + jobId + " - " + contextName + " - " + classPath)
//                val jobManager = getJobManagerForContext(Some(contextName), config, classPath)
//                jobManager match {
//                  case None =>
//                    logger.info("### JobManagerNotFound ! job : " + jobId)
//                    notFound(ctx, "JobManagerNotFound-" + jobId.toString)
//                  case _ =>
//                    jobManager.get ! DeleteJob(jobId)
//                    logger.info("### DONE ! user deleted job : " + jobId)
//                    ctx.complete(Map(StatusKey -> "DELETED"))
//                }
                jobInfo ! RemoveJobInfo(jobId)
                logger.info("### DONE ! user deleted job : " + jobId)
