...
class JobFileDAO(config: Config) extends JobDAO {
  ...
  private val deleteFlagText: String = "job-deleted-by-user"
...
//          jobs(jobInfo.jobId) = jobInfo
          jobInfo.contextName match {
            case `deleteFlagText` =>
              jobs.remove(jobInfo.jobId)
            case _ =>
              jobs(jobInfo.jobId) = jobInfo
          }
        }
...
//          configs(jobId) = jobConfig
  ...
  /**
    ...
    * @param jobId
    */
  override def removeJobInfo(jobId: String): Unit = {
    val jobInfo = getJobInfo(jobId)
    if(jobInfo.isDefined){
      //在文件系统中标识job已经删除
      writeJobInfo(jobsOutputStream, new JobInfo(
        jobId,
        deleteFlagText,
        jobInfo.get.jarInfo,
        jobInfo.get.classPath,
        jobInfo.get.startTime,
        jobInfo.get.endTime,
        jobInfo.get.error))
    }
    jobs.remove(jobId)
